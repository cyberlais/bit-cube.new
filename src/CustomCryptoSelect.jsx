...
import styles from "./CustomSelect.module.css"; // Импортируем стили

const CustomCryptoSelect = ({ options, onSelect, placeholder, disable }) => {
	const [selectedOption, setSelectedOption] = useState(options[0]);
	const [isOpen, setIsOpen] = useState(false);

	const handleOptionClick = (option) => {
		if (disable) return; // Блокируем выбор, если селект неактивен
	...
	return (
		<div className="relative">
			<div className="relative">
				<button
					type="button"
					onClick={() => !disable && setIsOpen(!isOpen)} // Открытие/закрытие списка только если селект активен
					...
					}`} // Меняем стиль при отключенном состоянии
					disabled={disable} // Прямое отключение кнопки
				>
					...
					{!disable && ( // Убираем стрелку, если селект неактивен
						<span
			...
			{!disable && ( // Скрываем выпадающий список, если селект неактивен
				<div
